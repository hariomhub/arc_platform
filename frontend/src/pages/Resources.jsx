...
import {
    ...
    Plus, AlertCircle, CheckCircle, Edit2, Shield, Loader2, RefreshCw
} from 'lucide-react';
...
const UPLOAD_ROLES   = ['founding_member','executive','professional'];
...
const ResourceModal = ({ resource, userRole, onClose, onSaved, showToast }) => {
    ...
    const [form, setForm] = useState({ title:resource?.title||'', description:resource?.description||'', abstract:resource?.abstract||'', demo_url:resource?.demo_url||'', category_slug:resource?.category_slug||'', type:resource?.type||'article', access_level:resource?.access_level||'public' });
    ...
    return (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }} onClick={e => e.target===e.currentTarget&&onClose()}>
            <div style={{ background:'white', borderRadius:'16px', width:'100%', maxWidth:'560px', maxHeight:'92dvh', overflow:'auto', boxShadow:'0 24px 64px rgba(0,0,0,0.22)' }}>
                <div style={{ padding:'1.4rem 1.75rem', borderBottom:'1px solid #e2e8f0', display:'flex', justifyContent:'space-between', alignItems:'center', background:'linear-gradient(135deg,#001a33,#003366)', borderRadius:'16px 16px 0 0' }}>
                    <h3 style={{ margin:0, fontSize:'1.05rem', color:'white', fontFamily:'var(--font-sans)', fontWeight:'700' }}>{isEdit?'✏️  Edit Resource':'📤  Upload New Resource'}</h3>
                    <button onClick={onClose} style={{ background:'rgba(255,255,255,0.15)', border:'none', cursor:'pointer', color:'white', borderRadius:'6px', padding:'0.3rem', display:'flex' }}><X size={18}/></button>
                </div>
                <form onSubmit={handleSubmit} style={{ padding:'1.75rem', display:'flex', flexDirection:'column', gap:'1.1rem' }}>
                    <div><label style={S.label}>Title *</label><input name="title" value={form.title} onChange={handleChange} style={S.input} placeholder="Resource title"/></div>
                    <div><label style={S.label}>Description *</label><textarea name="description" value={form.description} onChange={handleChange} rows={3} style={{...S.input,resize:'vertical'}} placeholder="Brief description…"/></div>
                    <div><label style={S.label}>Abstract <span style={{ fontWeight:400, color:'#64748b' }}>(optional)</span></label><textarea name="abstract" value={form.abstract} onChange={handleChange} rows={2} style={{...S.input,resize:'vertical'}} placeholder="Extended abstract…"/></div>
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(min(180px,100%),1fr))', gap:'1rem' }}>
                        <div><label style={S.label}>Type</label><select name="type" value={form.type} onChange={handleChange} style={S.input}>{allowedTypes.map(t => <option key={t} value={t}>{t.charAt(0).toUpperCase()+t.slice(1)}</option>)}</select></div>
                        <div><label style={S.label}>Access Level</label><select name="access_level" value={form.access_level} onChange={handleChange} style={S.input}><option value="public">Public</option><option value="registered">Member Only</option></select></div>
                    </div>
                    <div><label style={S.label}>Demo / Source URL <span style={{ fontWeight:400, color:'#64748b' }}>(optional)</span></label><input name="demo_url" value={form.demo_url} onChange={handleChange} style={S.input} placeholder="https://…"/></div>
                    <div>
                        <label style={S.label}>File <span style={{ fontWeight:400, color:'#64748b' }}>(PDF, Word, Excel, PowerPoint, Image, Video)</span></label>
                        <input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.jpg,.jpeg,.png,.gif,.webp,.svg,.mp4,.webm,.ogv,.mov,.avi" onChange={e => setFile(e.target.files[0])} style={{ fontSize:'0.85rem', color:'#475569', fontFamily:'var(--font-sans)' }}/>
                        {resource?.file_path && !file && <p style={{ margin:'0.3rem 0 0', fontSize:'0.78rem', color:'#64748b' }}>Current: <em>{String(resource.file_path).split('/').pop()}</em></p>}
                    </div>
                    {error && <div style={{ display:'flex', gap:'0.5rem', alignItems:'center', background:'#fef2f2', border:'1px solid #fecaca', borderRadius:'8px', padding:'0.65rem 0.9rem', color:'#dc2626', fontSize:'0.85rem' }}><AlertCircle size={15}/>{error}</div>}
...
const DeleteDialog = ({ onConfirm, onCancel }) => (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }} onClick={onCancel}>
        <div style={{ background:'white', borderRadius:'14px', padding:'clamp(1.5rem,3vw,2rem)', maxWidth:'380px', width:'100%', boxShadow:'0 20px 50px rgba(0,0,0,0.2)' }} onClick={e => e.stopPropagation()}>
            <div style={{ width:'50px', height:'50px', background:'#fef2f2', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 1rem', border:'2px solid #fecaca' }}><Trash2 size={22} color="#dc2626"/></div>
            <h3 style={{ margin:'0 0 0.6rem', color:'#1e293b', fontSize:'1.05rem', fontWeight:'700', textAlign:'center' }}>Delete Resource?</h3>
            <p style={{ margin:'0 0 1.5rem', color:'#64748b', fontSize:'0.88rem', textAlign:'center', lineHeight:'1.5' }}>This action cannot be undone. The file and all associated data will be permanently removed.</p>
    ...
    return (
        <div style={{ background:'white', borderRadius:'12px', border:'1px solid #e2e8f0', overflow:'hidden', boxShadow:'0 1px 4px rgba(0,0,0,0.04)', transition:'box-shadow 0.2s,transform 0.2s', display:'flex', flexDirection:'column' }}
            ...
            <div style={{ padding:'1rem 1.1rem', flex:1, display:'flex', flexDirection:'column', gap:'0.6rem' }}>
                ...
                {(resource.summary||resource.description) && <p style={{ margin:0, fontSize:'0.82rem', color:'#64748b', lineHeight:'1.5', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>{resource.summary||resource.description}</p>}
                <div style={{ display:'flex', gap:'0.4rem', flexWrap:'wrap', marginTop:'auto' }}>
                    {resource.category_slug && <span style={{ fontSize:'0.7rem', background:'#f1f5f9', color:'#475569', padding:'0.15rem 0.5rem', borderRadius:'100px', fontWeight:'500' }}>{resource.category_slug}</span>}
                    <span style={{ fontSize:'0.7rem', display:'inline-flex', alignItems:'center', gap:'3px', padding:'0.15rem 0.5rem', borderRadius:'100px', fontWeight:'600', background:resource.access_level==='public'?'#f0fdf4':'#fef9c3', color:resource.access_level==='public'?'#15803d':'#a16207' }}>
                        {resource.access_level==='public'?<Globe size={9}/>:<Lock size={9}/>}{resource.access_level==='public'?'Public':'Member'}
                    </span>
                    {resource.status && resource.status!=='approved' && <span style={{ fontSize:'0.7rem', padding:'0.15rem 0.5rem', borderRadius:'100px', fontWeight:'600', background:resource.status==='pending'?'#fef3c7':'#fef2f2', color:resource.status==='pending'?'#b45309':'#dc2626', textTransform:'capitalize' }}>{resource.status}</span>}
                </div>
            </div>
            <div style={{ padding:'0.7rem 1.1rem', borderTop:'1px solid #f1f5f9', display:'flex', alignItems:'center', gap:'0.5rem', background:'#fafafa', flexWrap:'wrap' }}>
                {!currentUser ? (
                    <a href="/login" style={{ display:'inline-flex', alignItems:'center', gap:'5px', background:'#475569', color:'white', border:'none', borderRadius:'6px', padding:'0.35rem 0.8rem', fontSize:'0.8rem', fontWeight:'700', cursor:'pointer', fontFamily:'var(--font-sans)', textDecoration:'none' }}><Lock size={12}/>Login to Download</a>
                ) : !canDownload ? (
                    <a href="/membership" style={{ display:'inline-flex', alignItems:'center', gap:'5px', background:'#0f766e', color:'white', border:'none', borderRadius:'6px', padding:'0.35rem 0.8rem', fontSize:'0.8rem', fontWeight:'700', cursor:'pointer', fontFamily:'var(--font-sans)', textDecoration:'none' }}><Lock size={12}/>Upgrade to Download</a>
                ) : (
                    <button onClick={() => onDownload(resource)} disabled={downloading} style={{ display:'inline-flex', alignItems:'center', gap:'5px', background:'#003366', color:'white', border:'none', borderRadius:'6px', padding:'0.35rem 0.8rem', fontSize:'0.8rem', fontWeight:'700', cursor:'pointer', fontFamily:'var(--font-sans)', opacity:downloading?0.6:1, transition:'background 0.15s' }} onMouseEnter={e => !downloading&&(e.currentTarget.style.background='#004080')} onMouseLeave={e => e.currentTarget.style.background='#003366'}>
                        {downloading?<Loader2 size={12} style={{ animation:'spin 1s linear infinite' }}/>:<Download size={12}/>}Download
                    </button>
                )}
                <span style={{ fontSize:'0.75rem', color:'#94a3b8', marginRight:'auto' }}>{resource.download_count||0} downloads</span>
                {canEdit && <button onClick={() => onEdit(resource)} style={{ display:'flex', alignItems:'center', justifyContent:'center', background:'#eff6ff', border:'1px solid #bfdbfe', borderRadius:'6px', padding:'0.33rem 0.55rem', cursor:'pointer' }} title="Edit"><Edit2 size={12} color="#1d4ed8"/></button>}
                {canDelete && <button onClick={() => onDelete(resource.id)} style={{ display:'flex', alignItems:'center', justifyContent:'center', background:'#fef2f2', border:'1px solid #fecaca', borderRadius:'6px', padding:'0.33rem 0.55rem', cursor:'pointer' }} title="Delete"><Trash2 size={12} color="#dc2626"/></button>}
            </div>
        </div>
    );
};
...
const Resources = () => {
    ...
    const handleDelete = async () => {
        try { await deleteResource(deleteId); setResources(prev => prev.filter(r => r.id!==deleteId)); showToast('Resource deleted.','success'); }
        catch (err) { showToast(getErrorMessage(err),'error'); }
        setDeleteId(null);
    };
        ...
        <div style={{ minHeight:'100vh', background:'#f8fafc', fontFamily:'var(--font-sans)' }}>
            <style>{`
                ...
                .res-collapse-btn {
                    ...
                    cursor: pointer; color: #64748B; box-shadow: 0 1px 2px rgba(0,0,0,0.04);
                ...
                /* Mobile: hide sidebar, show filter row */
            ...
            <div style={{ background:'linear-gradient(135deg,#002244 0%,#003366 55%,#005599 100%)', padding:'clamp(1.5rem,2.5vw,2rem) clamp(1rem,4vw,2rem) clamp(2.5rem,5vw,4rem)', position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ maxWidth:'720px', margin:'0 auto', textAlign:'center', position:'relative' }}>
                    ...
                    <h1 style={{ color:'white', fontSize:'clamp(1.6rem,4vw,2.5rem)', fontWeight:'800', margin:'0 0 0.75rem', fontFamily:'var(--font-serif,serif)', lineHeight:'1.2' }}>Research &amp; Resources</h1>
                    <p style={{ color:'rgba(255,255,255,0.7)', fontSize:'clamp(0.875rem,1.5vw,1rem)', margin:'0 0 0.6rem', lineHeight:'1.6' }}>Explore frameworks, whitepapers, videos, and AI governance resources curated by the AI Risk Council community.</p>
                    {user && (
                    ...
                    <div style={{ position:'relative', maxWidth:'520px', margin:'0 auto' }}>
                        ...
                        <input className="res-search-input" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search resources…"
                        ...
                        {search && <button onClick={() => setSearch('')} style={{ position:'absolute', right:'1rem', top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', color:'rgba(255,255,255,0.6)', display:'flex', padding:0 }}><X size={15}/></button>}
                    </div>
                </div>
            </div>
            ...
            <div className="res-body">
                    ...
                    {sidebarOpen && (
                        <div style={{ display:'flex', flexDirection:'column', gap:'1.25rem' }}>
                            <div>
                                ...
                                <button className={`res-filter-btn${filterType==='all'?' active':''}`} onClick={() => setFilterType('all')}>All Types{filterType==='all'&&<span style={{ marginLeft:'auto', width:'6px', height:'6px', borderRadius:'50%', background:'white' }}/>}</button>
                                {TYPE_OPTIONS.map(t => <button key={t} className={`res-filter-btn${filterType===t?' active':''}`} onClick={() => setFilterType(t)}>{TYPE_ICONS[t]}<span style={{ flex:1 }}>{t.charAt(0).toUpperCase()+t.slice(1)}</span><span style={{ fontSize:'0.72rem', opacity:0.65 }}>{typeCounts[t]||0}</span>{filterType===t&&<span style={{ width:'6px', height:'6px', borderRadius:'50%', background:'white', flexShrink:0 }}/>}</button>)}
                            </div>
                            <div>
                                ...
                                {[['all','All'],['public','Public'],['registered','Member Only']].map(([val,label]) => <button key={val} className={`res-filter-btn${filterAccess===val?' active':''}`} onClick={() => setFilterAccess(val)}>{val==='public'?<Globe size={13}/>:val==='registered'?<Lock size={13}/>:null}{label}</button>)}
                            </div>
                            <div style={{ padding:'0.6rem 0.7rem', background:'#f8fafc', borderRadius:'8px', fontSize:'0.78rem', color:'#64748b', fontWeight:'500' }}>{filtered.length} of {resources.length} resource{resources.length!==1?'s':''}</div>
                        </div>
                    )}
                </aside>
                ...
                <div className="res-main">
                    ...
                    <div className="res-mobile-filters">
                        ...
                        {TYPE_OPTIONS.map(t => <button key={t} onClick={() => setFilterType(filterType===t?'all':t)} style={{ padding:'5px 12px', borderRadius:'20px', border:'1px solid', borderColor:filterType===t?'#003366':'#CBD5E1', background:filterType===t?'#003366':'white', color:filterType===t?'white':'#475569', fontSize:'0.78rem', fontWeight:'600', cursor:'pointer', whiteSpace:'nowrap' }}>{t.charAt(0).toUpperCase()+t.slice(1)}</button>)}
                    </div>
                    ...
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'1.5rem', flexWrap:'wrap', gap:'0.75rem' }}>
                        ...
                        {user && <button onClick={() => setModal({mode:'create'})}
                    ...
                    {error && !loading && (
                        <div style={{ textAlign:'center', padding:'3rem 1rem', background:'white', borderRadius:'12px', border:'1px solid #fecaca' }}>
                            ...
                            <button onClick={fetchResources} style={{ display:'inline-flex', alignItems:'center', gap:'0.5rem', ...S.btnPrim }}><RefreshCw size={13}/>Retry</button>
                        </div>
                    )}

                    {loading && (
                        <div className="res-grid">
                            {[...Array(6)].map((_,i) => <div key={i} style={{ background:'white', borderRadius:'12px', border:'1px solid #e2e8f0', height:'200px', animation:'pulse 1.5s ease-in-out infinite' }}><div style={{ height:'3px', background:'#e2e8f0', borderRadius:'12px 12px 0 0' }}/></div>)}
                        </div>
                    )}
                    ...
                    {!loading && !error && filtered.length === 0 && (
                        <div style={{ textAlign:'center', padding:'5rem 1rem', background:'white', borderRadius:'12px', border:'1px dashed #cbd5e1', animation:'fadeIn 0.3s ease' }}>
                            ...
                            <p style={{ color:'#64748b', marginBottom:'0.75rem', fontSize:'0.95rem', fontWeight:'500' }}>{search?`No results for "${search}"`:resources.length===0?'No resources uploaded yet.':'No resources match your filters.'}</p>
                            {(search||filterType!=='all'||filterAccess!=='all') && <button onClick={() => { setSearch(''); setFilterType('all'); setFilterAccess('all'); }} style={{ border:'none', background:'none', color:'#003366', fontWeight:'700', fontSize:'0.875rem', cursor:'pointer', fontFamily:'var(--font-sans)', textDecoration:'underline' }}>Clear all filters</button>}
                        </div>
                    )}

                    {!loading && !error && filtered.length > 0 && (
                        <div className="res-grid" style={{ animation:'fadeIn 0.35s ease' }}>
                            {filtered.map(r => <ResourceCard key={r.id} resource={r} currentUser={user} onDownload={handleDownload} onEdit={res => setModal({mode:'edit',resource:res})} onDelete={setDeleteId} downloading={downloading[r.id]}/>)}
                        </div>
                    )}
                </div>
            </div>
