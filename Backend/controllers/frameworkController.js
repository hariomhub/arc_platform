    ...
    const guide = await Promise.all(
      phases.map(async (phase) => {
        ...
        return {
          ...
          icon: phase.icon,
          steps: steps.map(s => ({ step: s.step_number, title: s.title, desc: s.description }))
        };
      })
    );
...
export const createPillar = async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, message: 'Pillar created successfully', id: result.insertId });
  } catch (error) {
...
export const updatePillar = async (req, res) => {
  try {
    ...
    if (result.affectedRows === 0) return res.status(404).json({ success: false, message: 'Pillar not found' });
...
export const deletePillar = async (req, res) => {
  try {
    const [result] = await db.query('DELETE FROM framework_pillars WHERE id = ?', [req.params.id]);
    if (result.affectedRows === 0) return res.status(404).json({ success: false, message: 'Pillar not found' });
...
export const getAllPillarsAdmin = async (req, res) => {
  try {
    ...
    const parsedPillars = pillars.map(pillar => ({ ...pillar, tags: safeJSONParse(pillar.tags, []) }));
    res.json({ success: true, data: parsedPillars });
  } catch (error) {
...
export const createMaturityLevel = async (req, res) => {
  try {
    ...
    const userId = req.user.id;
    if (!level || !name || !description) return res.status(400).json({ success: false, message: 'Missing required fields' });
    const [result] = await db.query(
    ...
    res.status(201).json({ success: true, message: 'Maturity level created successfully', id: result.insertId });
  } catch (error) {
    console.error('Error creating maturity level:', error);
    if (error.code === 'ER_DUP_ENTRY') return res.status(400).json({ success: false, message: 'Maturity level number already exists' });
...
export const updateMaturityLevel = async (req, res) => {
  try {
    ...
    if (result.affectedRows === 0) return res.status(404).json({ success: false, message: 'Maturity level not found' });
...
export const deleteMaturityLevel = async (req, res) => {
  try {
    const [result] = await db.query('DELETE FROM framework_maturity_levels WHERE id = ?', [req.params.id]);
    if (result.affectedRows === 0) return res.status(404).json({ success: false, message: 'Maturity level not found' });
...
export const createPhase = async (req, res) => {
  try {
    ...
    const userId = req.user.id;
    if (!phaseNumber || !phaseLabel || !title || !duration) return res.status(400).json({ success: false, message: 'Missing required fields' });
    const [result] = await db.query(
    ...
    res.status(201).json({ success: true, message: 'Implementation phase created successfully', id: result.insertId });
  } catch (error) {
    console.error('Error creating phase:', error);
    if (error.code === 'ER_DUP_ENTRY') return res.status(400).json({ success: false, message: 'Phase number already exists' });
...
export const updatePhase = async (req, res) => {
  try {
    ...
    if (result.affectedRows === 0) return res.status(404).json({ success: false, message: 'Phase not found' });
...
export const deletePhase = async (req, res) => {
  try {
    const [result] = await db.query('DELETE FROM framework_implementation_phases WHERE id = ?', [req.params.id]);
    if (result.affectedRows === 0) return res.status(404).json({ success: false, message: 'Phase not found' });
...
export const createStep = async (req, res) => {
  try {
    ...
    const userId = req.user.id;
    if (!phaseId || !stepNumber || !title || !description) return res.status(400).json({ success: false, message: 'Missing required fields' });
    const [result] = await db.query(
    ...
    res.status(201).json({ success: true, message: 'Implementation step created successfully', id: result.insertId });
  } catch (error) {
...
export const updateStep = async (req, res) => {
  try {
    ...
    if (result.affectedRows === 0) return res.status(404).json({ success: false, message: 'Step not found' });
...
export const deleteStep = async (req, res) => {
  try {
    const [result] = await db.query('DELETE FROM framework_implementation_steps WHERE id = ?', [req.params.id]);
    if (result.affectedRows === 0) return res.status(404).json({ success: false, message: 'Step not found' });
...
export const createAuditTemplate = async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, message: 'Audit template created successfully', id: result.insertId });
  } catch (error) {
    console.error('Error creating audit template:', error);
    if (error.code === 'ER_DUP_ENTRY') return res.status(400).json({ success: false, message: 'Template ID already exists' });
...
export const updateAuditTemplate = async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      const [[old]] = await db.query('SELECT file_url FROM framework_audit_templates WHERE id = ?', [id]);
...
export const deleteAuditTemplate = async (req, res) => {
  try {
    ...
    const [[tmpl]] = await db.query('SELECT file_url FROM framework_audit_templates WHERE id = ?', [id]);
...
export const getAllAuditTemplatesAdmin = async (req, res) => {
  try {
    ...
    const parsedTemplates = templates.map(tmpl => ({ ...tmpl, fields: safeJSONParse(tmpl.fields, []) }));
    res.json({ success: true, data: parsedTemplates });
  } catch (error) {
